...
tmp_postgresql_connector_path = "#{Chef::Config[:file_cache_path]}/postgresql-#{node['aps-core']['mysql_driver']['version']}.jar"
...
remote_file tmp_mysql_connector_path do
  ...
  notifies :create, "file[#{final_mysql_connector_path}]", :immediately
  retries 2
end
...
remote_file tmp_postgresql_connector_path do
  ...
  notifies :create, "file[#{final_postgresql_connector_path}]", :immediately
  retries 2
end
